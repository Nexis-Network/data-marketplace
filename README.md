...
<h1 align="center">Nexis Marketplace</h1>

[![Build Status](https://github.com/Nexisprotocol/market/workflows/CI/badge.svg)](https://github.com/Nexisprotocol/market/actions)
[![Netlify Status](https://api.netlify.com/api/v1/badges/c85f4d8b-95e1-4010-95a4-2bacd8b90981/deploy-status)](https://app.netlify.com/sites/market-Nexisprotocol/deploys)
...
[![Test Coverage](https://api.codeclimate.com/v1/badges/da71759866eb8313d7c2/test_coverage)](https://codeclimate.com/github/Nexisprotocol/market/test_coverage)
[![js Nexisprotocol](https://img.shields.io/badge/js-Nexisprotocol-7b1173.svg)](https://github.com/Nexisprotocol/eslint-config-Nexisprotocol)
...
- [🦀 Data Sources](#-data-sources)
  - [Aquarius](#aquarius)
  - [Nexis Protocol Subgraph](#Nexis-protocol-subgraph)
  - [ENS](#ens)
  - [Purgatory](#purgatory)
...
The app is a React app built with [Next.js](https://nextjs.org) + TypeScript + CSS modules and will connect to Nexis remote components by default.
...
```bash
git clone git@github.com:Nexisprotocol/market.git
cd market
...
Using the `Nexis-market` with `barge` components is recommended for advanced users, if you are new we advice you to use the `Nexis-market` first with remote networks. If you prefer to connect to locally running components instead of remote connections, you can spin up [`barge`](https://github.com/Nexisprotocol/barge) and use a local Ganache network in another terminal before running `npm start`. To fully test all [The Graph](https://thegraph.com) integrations, you have to start barge with the local Graph node:

```bash
git clone git@github.com:Nexisprotocol/barge.git
cd barge

# startup with local Ganache and Graph nodes
./start_Nexis.sh --with-thegraph
```
...
To use the app together with MetaMask, importing one of the accounts auto-generated by the Ganache container is the easiest way to have test ETH available. All of them have 100 ETH by default. Upon start, the `Nexis_ganache_1` container will print out the private keys of multiple accounts in its logs. Pick one of them and import into MetaMask. Barge private key example : `0xc594c6e5def4bab63ac29eed19a134c130388f74f019bc74b8f4389df2837a58`
...
The `app.config.js` file is setup to prioritize environment variables for setting each Nexis component endpoint. By setting environment variables, you can easily switch between Nexis networks the app connects to, without directly modifying `app.config.js`.
...
All initial assets and their metadata (DDO) is retrieved client-side on run-time from the [Aquarius](https://github.com/Nexisprotocol/aquarius) instance, defined in `app.config.js`. All app calls to Aquarius are done with 2 internal methods which mimic the same methods in Nexis.js, but allow us:

- to cancel requests when components get unmounted in combination with [axios](https://github.com/axios/axios)
- hit Aquarius as early as possible without relying on any Nexis.js initialization
...
```tsx
import { QueryResult } from '@Nexisprotocol/lib/dist/node/metadatacache/MetadataCache'
...
### Nexis Protocol Subgraph

Most financial data in the market is retrieved with GraphQL from [our own subgraph](https://github.com/Nexisprotocol/Nexis-subgraph), rendered on top of the initial data coming from Aquarius.
...
For this our own [ens-proxy](https://github.com/Nexisprotocol/ens-proxy) is used, within the app the utility method `getEnsProfile()` is called as part of the `useProfile()` hook:
...
Based on [list-purgatory](https://github.com/Nexisprotocol/list-purgatory) some assets get additional data. Within most components this can be done with the internal `useAsset()` hook which fetches data from the [market-purgatory](https://github.com/Nexisprotocol/market-purgatory) endpoint in the background.
...
The latest deployment of the `main` branch is automatically aliased to `market.Nexisprotocol.com`, where the deployment on Netlify is the current live deployment.
...
- [**Ways to Contribute →**](https://docs.Nexisprotocol.com/concepts/contributing/)
- [Code of Conduct →](https://docs.Nexisprotocol.com/concepts/code-of-conduct/)
- [Reporting Vulnerabilities →](https://docs.Nexisprotocol.com/concepts/vulnerabilities/)
...
- The Nexis Protocol logo is a trademark of the Nexis Protocol Foundation and must be removed from forked versions of the market.
- The name "Nexis Market" is also copyright protected and should be changed to the name of your market.

Additionally, we would also advise that you retain the text saying "Powered by Nexis Protocol" on your forked version of the marketplace in order to give credit for the development work done by the Nexis Protocol team.

Everything else is made open according to the apache2 license. We look forward to seeing your data marketplace!

If you are looking to fork Nexis Market and create your own marketplace, you will find the following guides useful in our docs:

- [Forking Nexis Market](https://docs.Nexisprotocol.com/building-with-Nexis/build-a-marketplace/forking-Nexis-market)
- [Customising your Market](https://docs.Nexisprotocol.com/building-with-Nexis/build-a-marketplace/customising-your-market)
- [Deploying your Market](https://docs.Nexisprotocol.com/building-with-Nexis/build-a-marketplace/deploying-market)
...
This allocates the datatokens to the [dispenser contract](https://github.com/Nexisprotocol/contracts/blob/main/contracts/dispenser/Dispenser.sol) which dispenses data tokens to users for free. Publishers in your market will now be able to offer their assets to users for free (excluding gas costs).

## ✅ GDPR Compliance

Nexis Market comes with prebuilt components for you to customize to cover GDPR requirements. Find additional information on how to use them below.
...
```text
Copyright 2023 Nexis Protocol Foundation Ltd.
